  ...
  ******************************************************************************/
  this.getLog = function(request, response, next) {
    ...
    exec('tail --lines=50 ~/.pm2/logs/rpi-tracker-out-0.log', function(err, stdout, stderr) {
      ...
      console.error('Catastrophic error in appLogAPI.js/getLog(). Error: '+err.message);
    }
  };
    ...
    try {
      ...
      exec('pm2 flush', function(err, stdout, stderr) {
        ...
        if (err) {
          ...
          console.log('clearLog() successful. PM2 log cleared.');
        }
      ...
      console.error('Catastrophic error in appLogAPI.js/clearLog(). Error: '+err.message);
    }
  };
  ...
  this.backupLog = function() {
    ...
    try {
      ...
      //Create the bkup directory if it doesn't already exist.
      exec(['mkdir ~/.pm2/logs/bkup'], function(err, out, code) {
        ...
        var fileName = year+'-'+month+'-'+day+'-'+hour+'-'+minute+'-'+seconds;
      ...
      console.error('Catastrophic error in appLogAPI.js/backupLog(). Error: '+err.message);
    }
  };
