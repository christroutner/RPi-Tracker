...
function Constructor() {
  ...
  this.updateServer = function() {
    ...
    if(global.dataLog.logFileOpened) {
      ...
      globalThis.logTS('GPS signal not found.');
    }
  ...
  this.determineCorrectLogFile = function(fileName) {
    ...
    promiseCompareServerClientTimestamps.then( function(results) {
      ...
      console.log('server-interface.js/determineCorrectLogFile() clientTimeMS: '+clientTimeMS);
      console.log('server-interface.js/determineCorrectLogFile() serverTimeMS: '+serverTimeMS);
      ...
      console.log('globalThis.serverLastTimeStamp: '+globalThis.serverLastTimeStamp);
  ...
  this.findNextFile2 = function(serverLastTimeStamp) {
    ...
    fs.access('./assets/logfiles/'+fileName, fs.R_OK, function(err) {
      ...
      //Increase the time stamp to the next day and call this function again.
        ...
        serverLastTimeStamp.setDate(serverLastTimeStamp.getDate() + 1);
debugger;        
        //Create a Date object based on the current time, for comparison.
        var now = new Date();

        //If the serverLastTimeStamp gets increased beyond the current time, then set the time to now.
        //This prevents the calendar bug that I was seeing.
        if(serverLastTimeStamp.getTime() > now.getTime()) {
          serverLastTimeStamp = now;
          //return;
        }
      ...
      promiseLastTimeStamp.then( function(result) {
        ...
        fs.access('./assets/logfiles/'+fileName, fs.R_OK, function(err) {
          ...
          //The file does not exist.
            ...
            //If the serverLastTimeStamp gets increased beyond the current time, then return.
            //This prevents the calendar bug that I was seeing.
            if(serverLastTimeStamp.getTime() > now.getTime())
              return;
...
*/
  ...
  //Dev Note: This function can set up a race condition when debugging.
      ...
      promiseServerLastTimeStamp.then( function(results) {
        ...
        promiseClientLastTimeStamp.then( function(results) {
          ...
          */
          ...
          console.log('server-interface.js/compareServerClientTimestamps() globalThis.serverLastTimeStamp: '+globalThis.serverLastTimeStamp);
          ...
          var obj = {};
          obj.serverLastTimeStamp = globalThis.serverLastTimeStamp;
          obj.clientLastTimeStamp = globalThis.serverLastTimeStamp;
  ...
  this.getLastTimeStamp = function(fileName) {
    ...
    fs.readFile(global.dataLog.logFilePath+ fileName, 'utf8', function(err, data) { 
      if(err) {
        ...
        console.log('server-interface.js/getLastTimeStamp(): lastTimeStamp: '+lastTimeStamp);
  ...
  ******************************************************************************/
  this.startSync = function(request, response, next) {
    ...
    //Also send the IP address to the server.
  ...
  ******************************************************************************/
  this.stopSync = function(request, response, next) {
    ...
    //Also stop sending the IP address to the server.
    global.diagnostics.stopIpSync();
