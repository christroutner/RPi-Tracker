...
function Constructor() {
    ...
    fs.readFile('./data/'+ this.fileNameGeoJSONPoint, 'utf8', function(err, data) {
      if (err) {
        ...
        if( err.code == "ENOENT" ) {    
          ...
          this.jsonPointTimeStamp.fileRead = true;
          this.jsonPointTimeStamp.exists = false;
        ...
        this.jsonPointTimeStamp.data = JSON.parse(data);
        this.jsonPointTimeStamp.fileRead = true;
        this.jsonPointTimeStamp.exists = true;
      }
    });
  }
    ...
    fs.readFile('./data/'+this.fileNameGeoJSONLineString, 'utf8', function(err, data) {
      if (err) {
        ...
        if( err.code == "ENOENT" ) {    
          ...
          //Set flags for file handling.
          this.jsonLineString.fileRead = true;
          this.jsonLineString.exists = false;
        ...
        //debugger;
        //If the file already exists, the read it in.
        this.jsonLineString.data = JSON.parse(data);
        this.jsonLineString.fileRead = true;
        this.jsonLineString.exists = true;
      }
    });
  }
  ...
  this.logData = function() {
    ...
    this.jsonPointTimeStamp.data.features.push(
      { 
        "type": "Feature",
    ...
    this.jsonLineString.data.features[0].geometry.coordinates.push([formattedLong, formattedLat, 0.01]);
    //jsonFile.data.features[0].properties.timestamp.push(timeStamp);


    //Update the file every fileSaveCnt timer events.
    if( this.timerCnt >= fileSaveCnt ) {
      ...
      if(this.jsonPointTimeStamp.fileRead) {
        ...
        var kmlString = tokml(this.jsonPointTimeStamp.data);
      ...
      if(this.jsonLineString.fileRead) {
        ...
        //Convert the GeoJSON to KML
        kmlString = tokml(this.jsonLineString.data);
