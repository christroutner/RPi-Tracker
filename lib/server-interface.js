...
function Constructor() {
  ...
  this.trackerServerIp = '107.170.204.153';
  this.trackerServerPort = '3000';
  ...
  //this.userId = "57f58d97f084a502e58ef72d";
  ...
  this.syncWithServer = function() {
    ...
    if(serverDayStamp == clientDayStamp) {
      ...
      //Restore the timeout frequency of server calls to the original setting.
...
debugger;
      ...
      //if server is older, send the current log file.
      //if server is newer, send the next days log file.
      var candidate = globalThis.Date2DayStamp(new Date(serverDate.getTime()+(globalThis.logFileCnt)*msPerDay));
      var promiseLastTimeStamp = getLastTimeStamp(candidate+'-PT.json');
...
debugger;    
  ...
  //This function is called by findNextFile. The input is the filename of a log file and the function returns the last time stamp in that log file.
  //This function returns "" when the log file can't be found.
