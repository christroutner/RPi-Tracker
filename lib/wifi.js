  ...
  ******************************************************************************/
  ...
  this.restoreCheck = function() {
    ...
    try {
      ...
      //If the user has not configured the device with a CrumbShare account, then this block
      //becomes the default behavior: The device is reset into AP mode if the device is
      //rebooted 3 times without the user confirming the new change in WiFi settings.
      if(serverSettings.userId == "") {
      ...
      //If a CrumbShare account is configured, then the behavior changes:
      //The device tries to connect to the CrumbShare server the first time 
      //it is configured into WiFi Client mode. If it can not, then it reboots itself 
      //back into AP mode.
      } else {
        ...
        if(serverSettings.wifiType == "2") {
          ...
          console.log('WiFi Client mode detected. restoreCheck() executing.');
          ...
          var restoreCheckInterval = setInterval(function() {
            ...
            setTimeout(function() {
              ...
              if((global.diagnostics.localIp != undefined) && (global.diagnostics.globalIp != undefined)) {
      ...
      console.error('Error trying to execute wifi.js/restoreCheck()');
