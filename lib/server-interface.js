...
function Constructor() {
  ...
  this.syncWithServer = function() {
    ...
    //Send log files until the dates match.
      ...
      globalThis.determinCorrectLogFile(fileName);
  ...
  this.compareServerClientTimestamps = function(fileName) {
    debugger;
    console.log('compareServerClientTimestamps(), fileName = '+fileName);
    ...
    //get the last timestamp contained in the log file on the server.
      ...
      promiseServerLastTimeStamp.then( function(results) {
        ...
        if(error.indexOf('not found')) {
          console.log('Log file corresponding to '+globalThis.lastServerDate+' does not exist on server, so sending it.');
  ...
  this.getLastServerDate = function() {
    ...
    request(globalThis.trackerServerURL+'/api/userdata/'+serverSettings.userId, function (error, response, body) {
      ...
      if (!error && response.statusCode == 200) {
        ...
        if(globalThis.lastServerDate == data.geodata.lastDate) {
          debugger;
          
          //If the same date has been retrieved twice in a row
          var fileName = lastServerDate+'-PT.json';
          ...
          promiseGetServerLastTimeStamp.then( function(result) {
            ...
            globalThis.lastServerDate = result;
        ...
        //Update the lastServerDate global variable.
        } else {
