//This library contains code associated with logging GPS data to the hard drive.
...
function Constructor() {
  ...
  this.extendedDebug = false; //Temp code used for getting to the bottom of the startup logging bug.
  ...
  this.updateFileName = function() {
    ...
    if(global.gpsInterface.timeStamp == undefined) {
      if(global.debugState)
        console.error('global.gpsInterface.timeStamp == undefined. Can not update file name.')
        ...
        globalThis.jsonPointTimeStamp.data = 
        {
          ...
          [            
            { 
              "type": "Feature",
              //"geometry": {"type": "Point", "coordinates": [-122.36459, 48.30898]},
              "geometry": {"type": "Point", "coordinates": [global.gpsInterface.lastLong, global.gpsInterface.lastLat, 0.01]},
        ...
        { 
          ...
          [
            ...
            { 
              ...
              "geometry": {
                "type": "LineString",
                "coordinates": [[global.gpsInterface.lastLong, global.gpsInterface.lastLat, 0.01]]
              },
              "properties": {
                //"timestamp": [],
                "name": globalThis.docName,
  ...
  this.logData = function() {
    ...
    if(globalThis.logFileOpened) {
      ...
      if(isNaN(long) || isNaN(lat)) {
        ...
        if(global.debugState) {
          console.log('long or lat array has NaN values, skipping logging of data.');
          //console.log('Compiled lat = '+lat+', compiled long = '+long);
          //console.log('coodinateBuffer.length = '+global.gpsInterface.coordinateBuffer.length);
          //globalThis.extendedDebug = true;
        }
      ...
      if((long == null) || (lat == null)) {
        debugger;
        console.log('null values detected. lat = '+lat+', long = '+long);
      }
      ...
      if( globalThis.timerCnt >= globalThis.fileSaveCnt ) {
        ...
        if(globalThis.jsonPointTimeStamp.fileRead) {
          ...
          //6-27-17 CT: Saving data to backup files first to prevent loss on power-off race condition.
          var fileName = globalThis.fileNameGeoJSONPoint;
          var bkupFileName = fileName.slice(0,-5)+'-bkup'+fileName.slice(-5);
          
          //Write the backup file first.
          fs.writeFile('./assets/logfiles/'+bkupFileName, filePointTimeStampOutput, function (err) {
            if(err) {
              console.log('Error while trying to write GeoJSON Point file output.');
              console.log(err);
            } else {
              //Write the actual file.
              fs.writeFile('./assets/logfiles/'+fileName, filePointTimeStampOutput, function (err) {
          ...
          var fileName = globalThis.fileNameKMLPoint;
          var bkupFileName = fileName.slice(0,-4)+'-bkup'+fileName.slice(-4);
          
          //Write out the KML data
          fs.writeFile('./assets/logfiles/'+bkupFileName, kmlString, function (err) {
            if(err) {
              debugger;
              console.log('Error while trying to write KML Point file output.');
              console.log(err);
            } else {
              fs.writeFile('./assets/logfiles/'+fileName, kmlString, function (err) {
                if(err) {
                  debugger;
                  console.log('Error while trying to write KML Point file output.');
                  console.log(err);
        ...
        if(globalThis.jsonLineString.fileRead) {
          ...
          var fileName = globalThis.fileNameGeoJSONLineString;
          var bkupFileName = fileName.slice(0,-5)+'-bkup'+fileName.slice(-5);
          
          fs.writeFile('./assets/logfiles/'+bkupFileName, fileLineStringOutput, function (err) {
            if(err) {
              console.log('Error while trying to write GeoJSON LineString file output.');
              console.log(err);
            } else {
              fs.writeFile('./assets/logfiles/'+fileName, fileLineStringOutput, function (err) {
                if(err) {
                  console.log('Error while trying to write GeoJSON LineString file output.');
                  console.log(err);
                } else {
          ...
          var fileName = globalThis.fileNameKMLLineString;
          var bkupFileName = fileName.slice(0,-4)+'-bkup'+fileName.slice(-4);
