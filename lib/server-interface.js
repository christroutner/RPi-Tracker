...
function Constructor() {
  ...
  // 2/1/17 CT
  //Because of the new keep-out scrubbing feature on the Crumb Share server,
  //the sync can get stuck because the last date returned by the server does not match
  //the last date in the log file. This counter is used to detect this corner case and force
  //the RPi-Tracker to send the next days log file.
  this.sameServerDate = undefined;
  this.sameServerDateCnt = 0;
  this.forceNextDate = 0; //Flag used to signal if the next log file needs to be forced to be uploaded.
      ...
      promiseLastTimeStamp.then( function(result) {
        ...
        if(globalThis.lastServerDate >= result) {
          ...
          //the last log file.
            ...
            var curLogFile = globalThis.Date2DayStamp(new Date(serverDate.getTime()+(globalThis.logFileCnt+globalThis.forceNextDate)*msPerDay));
          }
...
debugger;
          ...
          //Wait 2 seconds to prevent a race condition.
          setTimeout(globalThis.checkUpload(fileName), 2000);
          
          /*
          //Get the updated server timestamp, but wait 2 seconds to prevent a race condition.
    ...
    request(globalThis.trackerServerURL+'/api/trackinglogfile/status/'+serverSettings.userId+'/'+filedate, function (error, response, body) {
      ...
      if (!error && response.statusCode == 200) {
        ...
        if(data.status == 'success') {
          ...
          //Clear the force date counter.
          globalThis.forceNextDate = 0;
