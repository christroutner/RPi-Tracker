    ...
    request.post({url: global.serverInterface.trackerServerURL+'/api/diagnostics/'+serverSettings.userId+'/update', 
      ...
      if (!error && response.statusCode == 200) {
        ...
        if(data.success) {
          ...
          debugger;
          //Update the server_settings.json with IP information
          var tmpObj = {};
          tmpObj.localIp = globalThis.localIp;
          tmpObj.globalIp = globalThis.globalIp;
          tmpObj.timestamp = globalThis.timestamp;
          serverSettings.ipData = tmpObj;
          ...
          fs.writeFile('./assets/server_settings.json', JSON.stringify(serverSettings, null, 4), function (err) {
            if(err) {
              console.log('Error in diagnostics.js/sendIp() while trying to write server_settings.json file.');
              console.log(err);
            } else {
              console.log('diagnostics.js/sendIp()() executed. server_settings.json updated with IP information.');
            }
          });
